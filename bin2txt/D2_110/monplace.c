...
static char *MonPlace_GetPlaceName(unsigned int id)
{
...
static int MonPlace_ConvertValue(void *pvLineInfo, char *acKey, unsigned int iLineNo, char *pcTemplate, char *acOutput)
{
    ST_LINE_INFO *pstLineInfo = pvLineInfo;

    if ( !stricmp(acKey, "code") )
    {
        if ( !String_BuildName(FORMAT(monplace), 0xFFFF, pcTemplate, NULL, pstLineInfo->vcode, MODULE_HAVENAME, acOutput) )
        {
...
int process_monplace(char *acTemplatePath, char *acBinPath, char *acTxtPath, ENUM_MODULE_PHASE enPhase)
{
    ...
    switch ( enPhase )
    {
        ...
        case EN_MODULE_SELF_DEPEND:
            m_iMonPlaceCount = 0;
            Lookup_MonPlace = MonPlace_GetPlaceName;
