...
static int MissCode_ConverterBin2Txt(unsigned char *pcBinBuf, unsigned char *pcTxtBuf)
{
    ...
    {
        case 0x04:
            if ( pcResult = Lookup_MissCalc(pcBinBuf[STACK_HEAD]) )
            {
                strcpy(pcTxtBuf, pcResult);
            }
            else
            {
                my_error("unknown miss calc in expression %u\r\n", m_uiMissCodeCount);
                result = 0;
            }
            break;
...
static int MissCode_StatArg2Proc(unsigned char *pcBinBuf, unsigned char *pcTxtBuf)
{
    ...
    if ( 0 != result )
    {
        ...
        {
            my_error("invalide itemstatcost %u in expression %u\r\n", uiSkillIndex, m_uiMissCodeCount);
            result = 0;
        }
    }
...
static int MissCode_ParseBin(char *acTemplatePath, char *acBinPath, char *acTxtPath)
{
    int result = 1;
    unsigned int i, j;
            ...
            case 0x00:
                ...
                if ( Stack_GetIndex(m_pvStack) > 1 )
                {
                    ...
                    while ( Stack_GetIndex(m_pvStack) > 1 )
                    {
                        ...
                        memset(acTempBuf, 0, sizeof(acTempBuf));
                        MissCode_TwoOprandProc("  ", SINGLE_OPERATER_LEVEL, EN_OPERATER_ARG_2, EN_OPERATER_LEFT, acTempBuf, sizeof(acTempBuf));
                    }
                }
...
int process_misscode(char *acTemplatePath, char *acBinPath, char *acTxtPath, ENUM_MODULE_PHASE enPhase)
{
    switch ( enPhase )
    {
