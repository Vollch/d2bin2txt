...
static unsigned int m_uiSkillDescBuffSize = 0;
static ST_SKILLDESCCODE *m_astSkillCode = NULL;
...
{
    if ( m_uiSkillDescCodeCount >= m_uiSkillDescBuffSize )
    {
        ST_SKILLDESCCODE *astNewArray = MemMgr_Realloc(m_astSkillCode, (sizeof(ST_SKILLDESCCODE) * (m_uiSkillDescBuffSize + 5000)));
        ...
        memset(&m_astSkillCode[m_uiSkillDescBuffSize], 0, sizeof(ST_SKILLDESCCODE) * 5000);
        m_uiSkillDescBuffSize += 5000;
    }

    if ( SkillDescCode_ConverterBin2Txt(pcTempBuf, m_astSkillCode[m_uiSkillDescCodeCount].acExpressionTxt) )
    {
        int iExpLen = strlen(m_astSkillCode[m_uiSkillDescCodeCount].acExpressionTxt);
        if ( iExpLen >= 255 )
        {
            my_printf("  Expression %u is %u characters long, consider refactoring it, or shortening related IDs\r\n", m_uiSkillDescCodeCount, iExpLen);
        }
...
static int SkillDescCode_ParseBin(char *acTemplatePath, char *acBinPath, char *acTxtPath)
{
    int result = 1;
    unsigned int i, j;
    ...
    m_astSkillCode = MemMgr_Malloc(sizeof(ST_SKILLDESCCODE) * 5000);
    memset(m_astSkillCode, 0, sizeof(ST_SKILLDESCCODE) * 5000);
    m_uiSkillDescBuffSize = 5000;
            ...
            case 0x00:
                ...
                if ( Stack_GetIndex(m_pvStack) > 1 )
                {
                    ...
                    while ( Stack_GetIndex(m_pvStack) > 1 )
                    {
                        ...
                        memset(acTempBuf, 0, sizeof(acTempBuf));
                        SkillDescCode_TwoOprandProc("  ", SINGLE_OPERATER_LEVEL, EN_OPERATER_ARG_2, EN_OPERATER_LEFT, acTempBuf, sizeof(acTempBuf));
                    }
                }
...
int process_skilldesccode(char *acTemplatePath, char *acBinPath, char *acTxtPath, ENUM_MODULE_PHASE enPhase)
{
    switch ( enPhase )
    {
