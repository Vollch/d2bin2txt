...
static int Shrines_FieldProc(void *pvLineInfo, char *acKey, unsigned int iLineNo, char *pcTemplate, char *acOutput)
{
    ...
    {
        if ( !String_BuildName(FORMAT(shrines), 0xFFFF, pcTemplate, NULL, iLineNo, NULL, acOutput) )
        {
...
static void Shrines_InitValueMap(ST_VALUE_MAP *pstValueMap, ST_LINE_INFO *pstLineInfo)
{
    INIT_VALUE_BUFFER;
    ...
    {
        VALUE_MAP_DEFINE_SIZED(pstValueMap, pstLineInfo, Niftyphrase, pad0x01[0], 2, TBL_STRING);
        ...
        VALUE_MAP_DEFINE_SIZED(pstValueMap, pstLineInfo, TreasureClass, pad0x52[84], 4, TREASURE);
...
int process_shrines(char *acTemplatePath, char *acBinPath, char *acTxtPath, ENUM_MODULE_PHASE enPhase)
{
    ...
    switch ( enPhase )
    {
        ...
        case EN_MODULE_SELF_DEPEND:
            Shrines_InitValueMap(pstValueMap, pstLineInfo);
