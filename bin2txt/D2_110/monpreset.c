...
static int MonPreset_ConvertValue(void *pvLineInfo, char *acKey, unsigned int iLineNo, char *pcTemplate, char *acOutput)
{
    ...
    if ( !stricmp(acKey, "Place") )
    {
        ...
        if ( 1 == pstLineInfo->bPlaceType && (pcResult = Lookup_MonStats(pstLineInfo->vPlace)) )
        {
            strcpy(acOutput, pcResult);
        }
        else if ( 0 == pstLineInfo->bPlaceType && (pcResult = Lookup_MonPlace(pstLineInfo->vPlace)) )
        {
            strcpy(acOutput, pcResult);
        }
        else if ( 2 == pstLineInfo->bPlaceType && (pcResult = Lookup_SuperUnique(pstLineInfo->vPlace)) )
        {
            strcpy(acOutput, pcResult);
        }
