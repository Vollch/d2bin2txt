...
static char *MissCalc_GetCalcCode(unsigned int id)
{
    if ( id >= m_iMissCalc )
    {
        return NULL;
...
static int MissCalc_ConvertValue(void *pvLineInfo, char *acKey, unsigned int iLineNo, char *pcTemplate, char *acOutput)
{
...
int process_misscalc(char *acTemplatePath, char *acBinPath, char *acTxtPath, ENUM_MODULE_PHASE enPhase)
{
    ...
    switch ( enPhase )
    {
        ...
        case EN_MODULE_SELF_DEPEND:
            m_iMissCalc = 0;
            Lookup_MissCalc = MissCalc_GetCalcCode;
